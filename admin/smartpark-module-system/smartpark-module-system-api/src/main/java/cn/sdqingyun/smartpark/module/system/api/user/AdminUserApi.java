...
public interface AdminUserApi extends AutoTransable<AdminUserRespDTO> {
    ...
    CommonResult<Long> create(@RequestBody AdminUserRespDTO reqVO);

    @PostMapping(PREFIX + "/update")
    @Operation(summary = "更新用户")
    void update(@RequestBody AdminUserRespDTO reqVO);

    @GetMapping(PREFIX + "/getUserByPhone")
    @Operation(summary = "通过手机号查询用户")
    @Parameter(name = "phone", description = "用户编号", example = "1", required = true)
    CommonResult<AdminUserRespDTO> getUserByPhone(@RequestParam("phone") String phone);
}
