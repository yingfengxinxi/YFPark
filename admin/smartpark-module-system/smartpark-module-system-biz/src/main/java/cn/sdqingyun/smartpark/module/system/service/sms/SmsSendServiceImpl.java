package cn.sdqingyun.smartpark.module.system.service.sms;
...
import cn.sdqingyun.smartpark.framework.common.util.sms.SendMsgRequest;
import cn.sdqingyun.smartpark.framework.common.util.sms.SendSmsUtils;
import cn.sdqingyun.smartpark.framework.datapermission.core.annotation.DataPermission;
import cn.sdqingyun.smartpark.module.system.framework.sms.core.client.SmsClient;
import cn.sdqingyun.smartpark.module.system.framework.sms.core.client.dto.SmsReceiveRespDTO;
import cn.sdqingyun.smartpark.module.system.framework.sms.core.client.dto.SmsSendRespDTO;
import cn.sdqingyun.smartpark.module.system.dal.dataobject.sms.SmsChannelDO;
import cn.sdqingyun.smartpark.module.system.dal.dataobject.sms.SmsTemplateDO;
...
import cn.sdqingyun.smartpark.module.system.mq.producer.sms.SmsProducer;
import cn.sdqingyun.smartpark.module.system.service.member.MemberService;
...
public class SmsSendServiceImpl implements SmsSendService {
    ...
    @Override
    public Long sendSingleSmsToOwner(String mobile, Long userId, String templateCode, Map<String, Object> templateParams) throws Exception {
        // 如果 mobile 为空，则加载用户编号对应的手机号

        // 执行发送
        return sendSms(mobile, userId, UserTypeEnum.OWNER.getValue(), templateCode, templateParams);
    }
    ...
    @Override
    public Long sendSms(String mobile, Long userId, Integer userType,
                        String templateCode, Map<String, Object> templateParams) throws Exception {
        // 校验短信模板是否合法
        SmsTemplateDO template = validateSmsTemplate(templateCode);
        // 校验短信渠道是否合法
        SmsChannelDO smsChannel = validateSmsChannel(template.getChannelId());
        ...
        if (isSend) {
            ...
            sendMsgRequest.setAccessKeyId(smsChannel.getApiKey());
            sendMsgRequest.setAccessKeySecret(smsChannel.getApiSecret());
            sendMsgRequest.setSignName(smsChannel.getSignature());
            sendMsgRequest.setPhoneNumber(mobile);
            JSONObject jsonObject = new JSONObject();
            for (String templateParam : templateParams.keySet()) {
                String value = String.valueOf(templateParams.get(templateParam));
                jsonObject.put(templateParam, value);
            }
            String templateParam = jsonObject.toString();
            sendMsgRequest.setTemplateCode(templateParam);
            ...
            if (StringUtils.equals("OK", code)) {
                ...
                //失败
                message = jsBody.get("message").toString();
            }
            smsLogService.updateSmsSendResult(sendLogId, success, code, message, "", "");
        }
        return sendLogId;
    }
