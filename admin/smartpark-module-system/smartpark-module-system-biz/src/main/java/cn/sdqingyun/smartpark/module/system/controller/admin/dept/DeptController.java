...
import cn.sdqingyun.smartpark.module.system.controller.admin.dept.vo.dept.DeptRespVO;
...
public class DeptController {
    ...
    @GetMapping(value = {"/list-all-simple-user", "/simple-list-user"})
    @Operation(summary = "获取部门精简信息下用户信息列表", description = "只包含被开启的部门下用户信息，主要用于前端的下拉选项")
    public CommonResult<List<DeptSimpleRespVO>> getDeptUserList() {
        List<DeptSimpleRespVO> list = deptService.getDeptUserList(
                new DeptListReqVO().setStatus(CommonStatusEnum.ENABLE.getStatus()));
        return success(list);
    }
