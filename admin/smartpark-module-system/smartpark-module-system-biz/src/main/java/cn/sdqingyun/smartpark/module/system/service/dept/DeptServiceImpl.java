...
import cn.sdqingyun.smartpark.module.system.dal.mysql.dept.DeptMapper;
...
public class DeptServiceImpl implements DeptService {
    ...
    @Override
    public List<DeptSimpleRespVO> getDeptUserList(DeptListReqVO reqVO) {
        List<DeptDO> list = deptMapper.selectList(reqVO);
        list.sort(Comparator.comparing(DeptDO::getSort));
        List<DeptSimpleRespVO> deptSimpleRespList = BeanUtils.toBean(list, DeptSimpleRespVO.class);
        if(CollectionUtils.isNotEmpty(deptSimpleRespList)){
            deptSimpleRespList.forEach(deptSimpleRespVO -> {
                Long id = deptSimpleRespVO.getId();
                LambdaQueryWrapperX<AdminUserDO>queryWrapperX=new LambdaQueryWrapperX<>();
                queryWrapperX.eq(AdminUserDO::getDeptId,id);
                queryWrapperX.eq(AdminUserDO::getStatus,"0");
                List<AdminUserDO> adminUserDOS = adminUserMapper.selectList(queryWrapperX);
                if(CollectionUtils.isNotEmpty(adminUserDOS)){
                    deptSimpleRespVO.setUserList(adminUserDOS);
                }
            });

        }
        return deptSimpleRespList;
    }
    ...
    public DeptDO getDeptByName(String name) {
        return deptMapper.selectByParentIdAndName(0L, name);
    }
}
