package cn.sdqingyun.smartpark.module.bus.service.energy;

import java.util.*;

import cn.sdqingyun.smartpark.module.bus.controller.admin.energy.vo.EnergyTaskRecordPageReqVO;
import cn.sdqingyun.smartpark.module.bus.controller.admin.energy.vo.EnergyTaskRecordSaveReqVO;
import cn.sdqingyun.smartpark.module.bus.dal.dataobject.energy.EnergyTaskRecordDO;
import jakarta.validation.*;
import cn.sdqingyun.smartpark.framework.common.pojo.PageResult;
import cn.sdqingyun.smartpark.framework.common.pojo.PageParam;

/**
 * 自定义抄表任务记录 Service 接口
 *
 * @author 管理员
 */
public interface EnergyTaskRecordService {

    /**
     * 创建自定义抄表任务记录
     *
     * @param createReqVO 创建信息
     * @return 编号
     */
    Long create(@Valid EnergyTaskRecordSaveReqVO createReqVO);

    /**
     * 更新自定义抄表任务记录
     *
     * @param updateReqVO 更新信息
     */
    void update(@Valid EnergyTaskRecordSaveReqVO updateReqVO);

    /**
     * 删除自定义抄表任务记录
     *
     * @param id 编号
     */
    void delete(Long id);

    /**
     * 获得自定义抄表任务记录
     *
     * @param id 编号
     * @return 自定义抄表任务记录
     */
    EnergyTaskRecordDO get(Long id);

    /**
     * 获得自定义抄表任务记录分页
     *
     * @param pageReqVO 分页查询
     * @return 自定义抄表任务记录分页
     */
    PageResult<EnergyTaskRecordDO> getPage(EnergyTaskRecordPageReqVO pageReqVO);

}