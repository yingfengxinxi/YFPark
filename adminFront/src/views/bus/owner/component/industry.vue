...
/** 行业分类标签 表单 */
defineOptions({ name: 'TagIndustryForm' })
...
const formRef_industry = ref() // 表单 Ref
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      const data = await TagIndustryApi.getTagIndustry(id)
      formData.value = data
...
const submitForm = async () => {
  // 校验表单
  await formRef_industry.value.validate()
  // 提交请求
  formLoading.value = true
  try {
    const data = formData.value as unknown as TagIndustryVO
    data.color = JSON.stringify(data.color)
    if (formType.value === 'create') {
      await TagIndustryApi.createTagIndustry(data)
      message.success(t('common.createSuccess'))
    } else {
      await TagIndustryApi.updateTagIndustry(data)
...
const resetForm = () => {
  ...
  formRef_industry.value?.resetFields()
}
</script>
