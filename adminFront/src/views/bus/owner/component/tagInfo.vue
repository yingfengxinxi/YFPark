...
/** 租客/业主标签 表单 */
defineOptions({ name: 'TagOwnerForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      const data = await TagOwnerApi.getTagOwner(id)
      formData.value = data
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as TagOwnerVO
    data.color = JSON.stringify(data.color)
    if (formType.value === 'create') {
      await TagOwnerApi.createTagOwner(data)
      message.success(t('common.createSuccess'))
    } else {
      await TagOwnerApi.updateTagOwner(data)
