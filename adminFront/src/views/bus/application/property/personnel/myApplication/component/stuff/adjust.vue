<template>
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="调整时间" prop="adjustTime">
        <el-date-picker
          v-model="queryParams.adjustTime"
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      <el-table-column label="单据编号" align="center" prop="processNumber" width="200">
      ...
      <el-table-column
        ...
        :formatter="tableColumnEmptyPlaceholder"
      />
      <el-table-column label="调整处理人" align="center" prop="cuserUid" width="180"
        ><template #default="scope">
...
import { PropertyStuffAdjustApi, PropertyStuffAdjustVO } from '@/api/bus/stuff/adjust'
import Form from '../../../../stuff/component/Form.vue'
import { useUserStore } from '@/store/modules/user'
const userStore = useUserStore()
/** 耗材业务调整 列表 */
defineOptions({ name: 'PropertyStuffAdjust' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<PropertyStuffAdjustVO[]>([]) // 列表的数据
...
const getList = async () => {
  ...
  try {
    ...
    const data = await PropertyStuffAdjustApi.getPropertyStuffAdjustPage(queryParams)
...
const handleDetail = async (row: any) => {
  approvalRef.value.open('stuff_adjust', row)
}
/** 重置按钮操作 */
const resetQuery = () => {
...
const openForm = (type: string, row?: any) => {
  formRef.value.open('stuff_adjust', type, row ? row.processCode : undefined, row)
}
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await PropertyStuffAdjustApi.deletePropertyStuffAdjust(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await PropertyStuffAdjustApi.exportPropertyStuffAdjust(queryParams)
    download.excel(data, '耗材业务调整.xls')
  } catch {
  } finally {
