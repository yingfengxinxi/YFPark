      ...
      <el-row :gutter="20">
        ...
        <el-col :span="8">
          <el-form-item label="报修人" prop="repairUid">
            <el-select v-model="formData.repairUid" placeholder="请选择报修人">
              <el-option
        ...
        <el-col :span="8">
          <el-form-item label="报修部门" prop="repairDepartmentId">
            <el-tree-select
              v-model="formData.repairDepartmentId"
              :data="deptTree"
              :props="defaultProps"
              filterable
              check-strictly
        ...
        <el-col :span="8">
          <el-form-item label="维修处理人" prop="operateUid">
            <el-select v-model="formData.operateUid" placeholder="请选择维修处理人" disabled>
              <el-option
                v-for="item in userOptions"
                :key="item.id"
                :label="item.nickname"
        ...
        ></el-col>
        <el-col :span="8">
          <el-form-item label="预计维修金额" prop="expectRepairPrice">
            <el-input-number
              :min="0"
              v-model="formData.expectRepairPrice"
              :step="1"
...
const dialogTitle = ref('资产维修') // 弹窗的标题
...
const formRules = reactive({
  repairUid: [{ required: true, message: '报修人不能为空', trigger: 'blur' }],
  operateUid: [{ required: true, message: '处理维修人不能为空', trigger: 'blur' }],
  repairDepartmentId: [{ required: true, message: '报修部门不能为空', trigger: 'blur' }],
...
const open = async (type?: any, listArray?: any, row?: any) => {
  ...
  formData.value.repairTime = new Date().getTime()
  if (type == 'detail') {
    formData.value = row
...
const submitForm = async () => {
  ...
  try {
    ...
    await PropertyApi.createRepairProperty(data)
