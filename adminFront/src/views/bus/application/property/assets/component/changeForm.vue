      ...
      <el-row :gutter="20">
        ...
        <el-col :span="8">
          <el-form-item label="变更后使用人" prop="afterUseUid">
            <el-select v-model="formData.afterUseUid" placeholder="请选择发起人">
              <el-option
        ...
        <el-col :span="8">
          <el-form-item label="变更后使用部门" prop="afterUseDepartmentId">
            <el-tree-select
              v-model="formData.afterUseDepartmentId"
              :data="deptTree"
              :props="defaultProps"
              filterable
              check-strictly
              default-expand-all
              node-key="id"
              placeholder="请选择变更后使用部门"
            /> </el-form-item
        ></el-col>
        <el-col :span="8">
          <el-form-item label="变更时间" prop="afterTime">
            <el-date-picker
        ...
        <el-col :span="8">
          <el-form-item label="变更处理人" prop="operateUid">
            <el-select v-model="formData.operateUid" placeholder="请选择变更处理人" disabled>
              <el-option
                v-for="item in userOptions"
                :key="item.id"
                :label="item.nickname"
...
const formRules = reactive({
  afterUseUid: [{ required: true, message: '变更后使用人不能为空', trigger: 'blur' }],
  operateUid: [{ required: true, message: '变更处理人不能为空', trigger: 'blur' }],
  afterUseDepartmentId: [{ required: true, message: '变更后使用部门不能为空', trigger: 'blur' }],
  afterTime: [{ required: true, message: '变更时间不能为空', trigger: 'blur' }]
})
...
const open = async (type?: any, listArray?: any, row?: any) => {
  ...
  resetForm()
  formData.value.afterTime = new Date().getTime()
...
const submitForm = async () => {
  ...
  try {
    ...
    data.muserUid = assetsList.value.map((item) => item.id).join(',')
    await PropertyApi.createChangeProperty(data)
