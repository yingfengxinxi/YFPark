<template>
      ...
      >
        ...
        <el-form-item>
          ...
          <el-button
            ...
            v-hasPermi="['bus:property-stuff-category:export']"
          >
  ...
  <!-- 表单弹窗：添加/修改 -->
  <CategoryForm ref="formRef" :CategoryList="Treedata" @success="getList" />
</template>
...
/** 资产耗材分类 列表 */
defineOptions({ name: 'PropertyStuffCategory' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<PropertyStuffCategoryVO[]>([]) // 列表的数据
...
const GetTreeData = async () => {
  const data = await PropertyStuffCategoryApi.getPropertyStuffCategoryTree()
  Treedata.value = data
}
/** 禁用 */
const forbidden = async (row: PropertyStuffCategoryVO) => {
  try {
    ...
    // 发起禁用
    await PropertyStuffCategoryApi.updatePropertyStuffCategory({
      ...row,
      flag: true,
...
const getList = async () => {
  loading.value = true
  try {
    const data = await PropertyStuffCategoryApi.getPropertyStuffCategoryPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    await message.delConfirm()
    // 发起删除
    await PropertyStuffCategoryApi.deletePropertyStuffCategory(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await PropertyStuffCategoryApi.exportPropertyStuffCategory(queryParams)
    download.excel(data, '资产耗材分类.xls')
  } catch {
  } finally {
