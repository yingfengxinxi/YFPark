...
/** 资产耗材分类 表单 */
defineOptions({ name: 'PropertyStuffCategoryForm' })
...
watch(
  ...
  (newValue) => {
    ...
    localCategoryList.value = [...newValue]
    localCategoryList.value.unshift({ id: 0, name: '无上级' })
  }
)
/** 打开弹窗 */
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await PropertyStuffCategoryApi.getPropertyStuffCategory(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as PropertyStuffCategoryVO
    if (formType.value === 'create') {
      data.status = 1
      await PropertyStuffCategoryApi.createPropertyStuffCategory(data)
      message.success(t('common.createSuccess'))
    } else {
      await PropertyStuffCategoryApi.updatePropertyStuffCategory(data)
