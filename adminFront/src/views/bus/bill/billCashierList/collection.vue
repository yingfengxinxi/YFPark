...
<template>
  ...
  <el-drawer v-model="collectShow" direction="rtl" size="1200px" title="收款" @close="cancelClick">
    <template #default>
            ...
            <span v-if="scope.row.index == 0">
              <div class="flex items-center justify-between">
                ...
                <el-button
                  ...
                  size="small"
                  type="primary"
                  @click="confirmClickMoneyAll(scope.row)"
                  >收完</el-button
                >
              </div>
            </span>
            <span v-else>
              <div class="flex items-center justify-between">
                <el-input-number
                  ...
                  :precision="2"
                  :min="0.0"
                  :max="scope.row.PendingCollection"
                  class="flex-1 m-r-10px"
                />
                <el-button plain size="small" type="primary" @click="confirmClickMoney(scope.row)"
                  >收完</el-button
                >
              </div>
            </span>
      ...
      <el-card class="!border-rd-4px w-100% box-border font-size-14px m-t-20px">
        <template #header>收款 </template>
        <el-form ref="form" :model="formData" label-width="120px">
          <el-row :gutter="20">
            <el-col :span="12">
              <el-form-item label="汇款方式：">
                <el-select v-model="formData.remitType" placeholder="">
                  <el-option
            ...
            <el-col :span="12">
              <el-form-item label="流水账户">
                <el-select v-model="formData.otherAccount" placeholder="">
                  <el-option
...
const open = async (roomInfo?: any, ownerInfo?: any, info?: any[]) => {
  console.log(roomInfo, '0000', ownerInfo)
  collectionList.value = []
  resetForm()
  // collectionList.value[0].data.slice(0, 0)
  console.log(collectionList.value, 'info')
  if (info.data?.length)
  ...
  formData.value.buildId = roomInfo?.buildId
  formData.value.villageId = roomInfo?.villageId
  formData.value.roomNumber = roomInfo?.id
  formData.value.roomNumber = roomInfo?.id
  ...
  formData.value.remitType = getStrDictOptions(DICT_TYPE.REMIT_TYPE)[0].value
}
...
const getOrgAccountList = async () => {
  const data = await BuildApi.orgAccountList({
    pageNo: 1,
    pageSize: 10,
    build: formData.value.buildId
  })

  OrgAccountList.value = data
  formData.value.otherAccount = data[0].id
}
...
const confirmClick = async () => {
  ...
  collectionList.value[0].data.forEach((item) => {
    if (item.money) {
      data.push({
        ...
        receivablePayableAmount: item.receivablePayableAmount,
        payDate: item.payDate,
        payStartDate: item.payStartDate,
        payEndDate: item.payEndDate,
        receivablePayment: item.receivablePayment,
        thisPayment: item.money,
  ...
  if (data.length == 0) {
    message.error('请选择收款金额')
    return false
  }
  formData.value.billCollectionList = data
  console.log(formData, 'formData')
  formLoading.value = true
  try {
    await orgBillStreamApi.orgBillStreamCreate(formData.value)
    emit('success')
    collectShow.value = false
  } finally {
...
const confirmClickMoney = async (row) => {
  if (row.money < row.PendingCollection) {
    row.money = row.PendingCollection
    totalMoney.value = (Number(totalMoney.value) + row.PendingCollection).toFixed(2)
  }
}
const confirmClickMoneyAll = async (row) => {
  row.money = row.PendingCollection
  totalMoney.value = 0.0
  collectionList.value[0].data.forEach((element) => {
    element.money = element.PendingCollection
    totalMoney.value = (Number(totalMoney.value) + Number(element.PendingCollection)).toFixed(2)
  })
}
