<template>
  ...
  >
    ...
    >
      <el-form-item label="收据编号名称" prop="name">
        <el-input v-model="formData.name" maxlength="5" min="0" placeholder="请输入收据编号名称" />
      </el-form-item>
      ...
      <el-row gutter="20">
        <el-col :span="12">
          <el-form-item label="开始编号" prop="startNumber">
            <el-input v-model="formData.startNumber" placeholder="请输入开始编号如0000001" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="结束编号" prop="endNumber">
            <el-input v-model="formData.endNumber" placeholder="请输入结束编号如9999999" />
          </el-form-item>
        </el-col>
      </el-row>
...
const formRules = reactive({
  name: [{ required: true, message: '收据编号不能为空', trigger: 'blur' }],
  startNumber: [{ required: true, message: '开始编号不能为空', trigger: 'blur' }],
  endNumber: [{ required: true, message: '结束编号不能为空', trigger: 'blur' }],
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await RuleApi.get(id)
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as RuleVO
