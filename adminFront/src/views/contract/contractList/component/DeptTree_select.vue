...
-->
<template>
  <div class="head-container" v-loading="selectLoading">
    <el-tree
      :data="deptList"
      ref="treeRef"
    ...
    >
      <template #default="{ node, data }">
        <div class="slot-t-node flex w-full">
          ...
          <Icon
            class="m-r-[6px] transform-translate-y-[1.5px]"
            :icon="
              node.level == 1
                ? 'fa:bank'
                : node.level == 2
    ...
    <el-table :data="selectList" v-if="!show_tree">
      ...
      <el-table-column prop="rentalArea" label="面积" />
    </el-table>
  </div>
</template>
  ...
  await data.checkedNodes.forEach(async (event) => {
    if (event.level == 2) {
      ...
      BuildApi.getRoomListByLayerId({
        ...
        console.log(level_event, 'level_event')
        await level_event.forEach((level_eventitem) => {
          if (props.notdis) {
            if (
              level_eventitem.roomStatus == 10 &&
              !containsObject(selectList.value, level_eventitem)
            ) {
              level_eventitem.level1_key = event.level1_key
              level_eventitem.level2_key = event.level_key
              level_eventitem.level_key = `3-${level_eventitem.id}`
              selectList.value.push(level_eventitem)
...
const getDeptTreeSelect = async (data, change) => {
  ...
  await data.forEach((event) => {
    ...
    Area += Number(event.rentalArea)
  })
  console.log(data, 'data', Area)
  setTree(selectIdList, showKeyList)
}
defineExpose({ getDeptTreeSelect })
...
const setTree = async (selectIdList, showKeyList) => {
  console.log(showKeyList, '======================')
  buildList.value = []
  deptList.value.forEach((event) => {
    buildList.value.push(`0-${event.id}`)
  })
  try {
    //将父级节点放入buildList,默认展开的父级节点
