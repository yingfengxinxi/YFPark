...
/** 合同标签 表单 */
defineOptions({ name: 'TagContractForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      const data = await TagContractApi.getTagContract(id)
      formData.value = data
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as TagContractVO
    data.color = JSON.stringify(data.color)
    if (formType.value === 'create') {
      await TagContractApi.createTagContract(data)
      message.success(t('common.createSuccess'))
    } else {
      await TagContractApi.updateTagContract(data)
