...
/** 房源标签 表单 */
defineOptions({ name: 'TagHouseForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      const data = await TagHouseApi.getTagHouse(id)
      formData.value = data
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as TagHouseVO
    data.color = JSON.stringify(data.color)
    if (formType.value === 'create') {
      await TagHouseApi.createTagHouse(data)
      message.success(t('common.createSuccess'))
    } else {
      await TagHouseApi.updateTagHouse(data)
