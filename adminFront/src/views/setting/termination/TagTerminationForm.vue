...
/** 退租原因标签 表单 */
defineOptions({ name: 'TagTerminationForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      const data = await TagTerminationApi.getTagTermination(id)
      formData.value = data
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as TagTerminationVO
    data.color = JSON.stringify(data.color)
    if (formType.value === 'create') {
      await TagTerminationApi.createTagTermination(data)
      message.success(t('common.createSuccess'))
    } else {
      await TagTerminationApi.updateTagTermination(data)
