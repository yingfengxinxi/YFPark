...
/** 项目标签 表单 */
defineOptions({ name: 'TagVillageForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      const data = await TagVillageApi.getTagVillage(id)
      formData.value = data
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as TagVillageVO
    data.color = JSON.stringify(data.color)
    if (formType.value === 'create') {
      await TagVillageApi.createTagVillage(data)
      message.success(t('common.createSuccess'))
    } else {
      await TagVillageApi.updateTagVillage(data)
