...
export const getLastPath = (childRaw: object, path: string) => {
  // console.log(childRaw)
  if (childRaw) {
    const childPath = pathResolve(path, childRaw[0].path)
    if (!childRaw[0].children) {
      return `${childPath}`
    } else {
      const lastpath = getLastPath(childRaw[0].children, childPath)
      return `${lastpath}`
    }
