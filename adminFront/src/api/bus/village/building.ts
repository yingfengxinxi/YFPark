...
export interface BuildVO {
  ...
  countryId?: number | null // 国家ID
  provinceId?: number | null // 省份ID
  cityId?: number | null // 城市ID
  districtId?: number | null // 区县ID
  streetId?: number | null // 街道/乡镇ID
  communityId?: number | null // 社区/村ID
  ...
  extraConfig?: {
    completedTime?: Date | null //竣工时间
    floor_height?: string | null //标准层高
    ownerName?: string | null //物业
    ownerCost?: string | null //物业费
    parkingNumber?: string | null //车位数量
    parkingMonthlyRent?: string | null //车位月租金
    airConditioner?: string | null //空调
    airConditionerCost?: string | null //空调费
    airConditionerTime?: string | null //空调开放时间
    elevator?: string | null //电梯
    network?: string | null //网络
    settledEnterprise?: string | null //入驻企业
    post_txt?: string | null //招商岗位
    department_txt?: string | null //招商部门
    attractInvestmentTime?: string | null //招商时间
  } // 默认配置
  tagInfo?: string | null // 楼宇标签
  isHot?: number | null // 是否热门
  sort?: number | null // 排序，越大越前
  status?: number | null // 数据状态(1使用，0隐藏)
...
export const BuildApi = {
  ...
  // 剖面图查询  /admin-api/bus/build/projectProfile {"id": 15194,"layerId": 15194}
  projectProfile: async (data) => {
    return await request.post({
      url: `/bus/build/projectProfile`,
      data
    })
  },

  // 项目楼宇列表 列表   /admin-api/bus/build/getBuildList
  getBuildList: async (params) => {
    return await request.get({
      url: `/bus/build/getBuildList`,
      params
    })
  }
}
