...
import { MenuTop } from '@/layout/components/MenuTop'
import { MenuLeft } from '@/layout/components/MenuLeft'
...
// 二级导航
const ShowTwoMenu = computed(() => appStore.ShowTwoMenu)
...
export const useRenderLayout = () => {
  ...
  // logo class={[{ 'custom-hover': topHeaderBgColor.value.length > 13 }]}
  const renderTopLeftBottom = () => {
    return (
      <>
        <div
          class={[
            ...
            {
              'gradient-bg': topHeaderBgColor.value.length > 13,
              'bg-[var(--top-header-bg-color)]': topHeaderBgColor.value.length <= 13
            },
            {
              'layout-border__bottom': !tagsView.value
            }
        ...
        >
          ...
          <MenuTop class="h-[var(--top-tool-height)] flex-1 px-10px"></MenuTop>
          <ToolHeader></ToolHeader>
        </div>
        <div class="absolute left-0 top-[var(--logo-height)+1px] h-[calc(100%-1px-var(--logo-height))] w-full flex">
          {ShowTwoMenu.value ? (
                ...
                <TagsView
                  class={[
                    ...
                    ShowTwoMenu.value
                      ...
                      : {
                          '!fixed top-0 left-0 z-10': fixedHeader.value,
                          'mt-[calc(var(--logo-height)+1px)]': collapse.value && fixedHeader.value,
                          'mt-[calc(var(--logo-height)+1px)]': !collapse.value && fixedHeader.value
                        }
                  ]}
                  style="transition: width var(--transition-time-02), left var(--transition-time-02);"
